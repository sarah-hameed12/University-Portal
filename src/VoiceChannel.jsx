...
import { FiMic, FiMicOff, FiUser, FiUsers, FiX, FiHeadphones, FiVolume2 } from 'react-icons/fi';
...
const VoiceChannel = ({ currentUser }) => {
  ...
  const logEvent = (event) => {
    setDebug(prev => [...prev.slice(-10), `[${new Date().toLocaleTimeString()}] ${event}`]);
  ...
  useEffect(() => {
    const fetchChannelDetails = async () => {
      try {
        const response = await axios.get(`http://127.0.0.1:8001/api/feed/voice-channels/${channelId}/`);
  ...
  useEffect(() => {
    ...
    const initUserMedia = async () => {
      try {
        ...
        alert("Could not access microphone. Voice chat requires microphone permissions.");
        navigate(`/communities/${communityId}`);
      }
    };
    ...
    const initWebSocket = (stream) => {
      ...
      const wsUrl = `${wsScheme}://127.0.0.1:8001/ws/voice/${channelId}/?user_id=${currentUser.id}&display_name=${encodeURIComponent(currentUser.email || currentUser.id)}`;
      ...
      socket.onopen = () => {
        ...
        axios.post(`http://127.0.0.1:8001/api/feed/voice-channels/${channelId}/participants/`, {
          user_id: currentUser.id,
          display_name: currentUser.email || currentUser.id
        }).catch(err => console.error('Error registering as participant:', err));
      };
    ...
    const handleSocketMessage = (data, stream) => {
      switch (data.type) {
        ...
        case 'mute_change':
          logEvent(`User ${data.user_id} ${data.is_muted ? 'muted' : 'unmuted'}`);
    ...
    return () => {
      ...
      if (channelId && currentUser?.id) {
        axios.delete(`http://127.0.0.1:8001/api/feed/voice-channels/${channelId}/?user_id=${currentUser.id}`)
          .then(response => {
  ...
  const handleUserJoined = (userId, displayName, stream) => {
    ...
    peerConnection.onconnectionstatechange = () => {
      logEvent(`Connection state with ${userId}: ${peerConnection.connectionState}`);
    };
    
    peerConnection.oniceconnectionstatechange = () => {
      logEvent(`ICE connection state with ${userId}: ${peerConnection.iceConnectionState}`);
  ...
  const handleOffer = async (fromUserId, offer, stream) => {
    try {
      ...
      if (!peerConnectionsRef.current[fromUserId]) {
        ...
        // Monitor connection state
        peerConnection.onconnectionstatechange = () => {
          logEvent(`Connection state with ${fromUserId}: ${peerConnection.connectionState}`);
        };
        
        peerConnection.oniceconnectionstatechange = () => {
          logEvent(`ICE connection state with ${fromUserId}: ${peerConnection.iceConnectionState}`);
        };
      }
        ...
        <div className={styles.participantsContainer}>
          ...
          <div className={styles.participantsList}>
            ...
            <div className={styles.participant}>
              <div className={`${styles.participantIcon} ${activeSpeakers[currentUser?.id] ? styles.speaking : ''}`}>
              ...
              <div className={styles.participantInfo}>
                ...
                {isMuted && <div className={styles.muteBadge}><FiMicOff /></div>}
              </div>
            </div>
            ...
            {participants
              ...
              .map(participant => (
                <div key={participant.user_id} className={styles.participant}>
                  <div className={`${styles.participantIcon} ${activeSpeakers[participant.user_id] ? styles.speaking : ''}`}>
                    {activeSpeakers[participant.user_id] ? <FiVolume2 /> : <FiUser />}
                  </div>
                  <div className={styles.participantInfo}>
                    <div className={styles.participantName}>
                      {participant.display_name}
                    </div>
                    {participant.is_muted && <div className={styles.muteBadge}><FiMicOff /></div>}
                  </div>
                </div>
              ))}
          </div>
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className={styles.debugPanel}>
          ...
          <div className={styles.debugInfo}>
            <div>Connection Status: {socketRef.current?.readyState === 1 ? 'Connected' : 'Disconnected'}</div>
            ...
            <div className={styles.debugLog}>
              {debug.map((log, i) => (
                <div key={i} className={styles.debugLogItem}>{log}</div>
              ))}
            </div>
            <button onClick={() => setDebug([])} className={styles.clearButton}>Clear Log</button>
          </div>
        </div>
      )}
    </div>
