...
function Single({request, acceptor, showprof}) {
    ...
    const [imgSrc, setImgSrc] = useState(null);
        ...
        useEffect(() => {
        fetch(`http://localhost:5001/download/${acceptor}`).then((response) => {
            if (response.ok) {
    ...
    const {time = 'N/A', purpose = 'No purpose specified.', place = 'Unnamed Location'} = request;

    const handleAcceptClick = (e) => {
        e.preventDefault();
        console.log("Accept button clicked");
        fetch(`http://localhost:5001/approved/${acceptor}/${request.req_id}`).then((response) => {
            if(response.ok) {
    ...
    return (
        <article className="acceptance-card" aria-labelledby={`accepter-name-${acceptor}`}>
          {/* 1. Accepter Info Section (Left) */}
          <div className="accepter-info">
            <img src={imgSrc} alt={`${acceptor}'s avatar`} className="accepter-avatar" onClick={(e)=>{
                e.stopPropagation();
                showprof(acceptor);
            }} />
            <span id={`accepter-name-${acceptor}`} className="accepter-name">{acceptor}</span>
            <span className="accepter-prompt">wants to join:</span>
          </div>
    
          {/* 2. Request Details Section (Center) */}
          <div className="request-details">
            <p className="detail-purpose" title={purpose}>
               "{purpose.length > 60 ? `${purpose.substring(0, 60)}...` : purpose}" {/* Truncate long purpose */}
            </p>
