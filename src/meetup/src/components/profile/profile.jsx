...
function Profile_comp({ close_func, profile_info, profile_id, edit, req}) {
    ...
    let default_img = profile_info.img || "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-978409_1280.png"
    ...
    useEffect(() => {
        console.log("start")
        fetch(`http://localhost:5001/download/${profile_id}`)
            .then((res) => {
                console.log("here");
        ...
        fetch(`http://localhost:5001/download-text/${profile_id}`).then((res) => {
            console.log("text");
            if (!res.ok) throw new Error("File not found");
            return res.json();
        }).then((data) => {
    ...
    const submit_handler = async (event) => {
        ...
        function dataURLtoBlob(dataURL) {
            if (typeof dataURL !== 'string') {
              throw new TypeError('Expected dataURL to be a string, got ' + typeof dataURL);
            }
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5001/upload_profile/${id}`, {
                method: "POST",
                body: formDataa,
            });
            ...
            if (!response.ok) {
                ...
                } catch (e) { /* Ignore if parsing response body fails */ }
            ...
            <div className="profile-info">
                <form onSubmit={submit_handler} className="my_frm1">
                        ...
                        {is_edditing ? (
                            <>
                                <div className="picture">
                                    <img src={curr_url1} alt="not found" onClick={img_click} className="pic" />
                                    <input type="file"
                                        name="img1"
                                        ref={fileInputRef}
                    ...
                    <div className="profile-detail">
                        {
                            is_edditing ? (
                                <>
                                    ...
                                    <div className="form_group">
                                        <label htmlFor="major"><strong>major: </strong></label>
                                        <input type="text"
                                            name="major"
                                            id="major"
                                ...
                                : (<>
                                    ...
                                    <p><strong>Major: </strong> {profile_data.major}</p>
                                    <p><strong>Year: </strong> {profile_data.year}</p>
                    ...
                    </div>
                    {
                        ...
                        (<div className="socials">
                            ...
                            {profile_info.socials.insta && (
                                <a href={profile_info.socials.insta} target="_blank" rel="noopener noreferrer">Instagram</a>
                            )}

                            {profile_info.socials.fb && (
                                <a href={profile_info.socials.fb} target="_blank" rel="noopener noreferrer">Facebook</a>
                            )}

                            {profile_info.socials.lkdin && (
                                <a href={profile_info.socials.lkdin} target="_blank" rel="noopener noreferrer">Linkedin</a>
                            )}
                        </div>)
                    }
                    <div className="profile_actions">
                        {is_edditing ? (
                            <>
                                <button type="button" onClick={press_cancel} className="button-cancel">Cancel</button>
                                <button type="submit" className="button-save">Save Changes</button>
                            </>
                        ) : (
                            <>
                                {edit && <button type="button" onClick={press_edit} className="button-edit">Edit Profile</button>}
                                <button type="button" onClick={(e)=>{
                                    console.log("ending",req)
                                    close_func(req)}} className="button-close">Close</button>
                            </>
                        )}
                    </div>
                </form>
