...
function Post({ close_func, zoom_func, zoomed, request, showprof, editp }) {
  ...
  useEffect(() => {
    fetch(
      `https://meetupserver-production.up.railway.app/download/${request.id}`
    )
      .then((response) => {
        if (response.ok) {
  ...
  if (!request || !request.location?.lat || !request.location?.lng) {
    console.error(
      "Post component received invalid or incomplete request data:",
      request
    );
    return (
      <div className="post-error">
        Error: Could not display post due to missing data.
      </div>
    );
  }
  console.log(request);
  ...
  const handleAccept = () => {
    ...
    fetch(
      `https://meetupserver-production.up.railway.app/accept_request/${id}/${request.req_id}`
    )
      .then((response) => {
        if (response.ok) {
  ...
  if (zoomed) {
    return (
      ...
      >
        <div
          className="zoomed_post_modal"
          role="dialog"
          aria-modal="true"
          aria-labelledby="zoomedPostTitle"
          onClick={(e) => e.stopPropagation()}
        >
          <button
            className="zoomed_close_button"
            onClick={close_func}
          ...
          <div className="zoomed_post_content">
            <div className="zoomed_sender_info">
              <img
                src={imgSrc}
                alt={`${name}'s avatar`}
                className="zoomed_avatar"
                onClick={(e) => {
                  e.stopPropagation();
                  showprof(request.id, false, request);
                }}
              />
              <h2 id="zoomedPostTitle" className="zoomed_name">
                {name}
              </h2>
            </div>

            <div className="zoome/default-avatar.pngd_details_section">
              <p className="zoomed_detail_item">
                <span className="detail_label">Time:</span> {time}
              </p>
              <p className="zoomed_detail_item">
                <span className="detail_label">Group Size:</span> {group_size}
              </p>
              <p className="zoomed_purpose_full">
                <span className="detail_label">Purpose:</span> {purpose}
              </p>
              <p className="zoomed_detail_item">
                <span className="detail_label">Location:</span> {place}
              </p>
            </div>
          ...
          {editp && (
            <div className="zoomed_post_actions">
              <button className="button button-decline" onClick={handlereject}>
                Decline
              </button>
              <button className="button button-accept" onClick={handleAccept}>
                Accept Request
              </button>
            </div>
          )}
