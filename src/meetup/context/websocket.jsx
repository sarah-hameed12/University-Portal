...
export const WebsocketProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const func = async () => {
      ...
      console.log("done");
      socketref.current = io("https://meetupserver-production.up.railway.app", {
      ...
      socketref.current.on("my_requests", (data) => {
        ...
        set_requets(() => {
          ...
          let temp = data.filter(
            (request) => !n_inter.includes(request.req_id)
          );
  ...
  useEffect(() => {
    const func = async () => {
      if (id !== null) {
        console.log("Fetching profile for email:", id);
        try {
          const response = await axios.get(
            `http://127.0.0.1:8000/api/profile/?email=${encodeURIComponent(id)}`
          );
  ...
  const removeRequest = useCallback((req_id) => {
    ...
    set_requets((datas) => {
      let temp = datas.filter(
        (request) => !interacted.current.includes(request.req_id)
      );
      console.log("temp", temp);
      return temp;
    });

    fetch(`https://meetupserver-production.up.railway.app/request/${req_id}`)
      .then((response) => {
      ...
      .then((data) => {
        ...
        setinteracted((old_requests) => {
          return [
            ...old_requests.filter((request) => data.req_id !== request.req_id),
            data,
          ];
        });
